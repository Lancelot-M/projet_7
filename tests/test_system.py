...
def test_questionning_return_dict(monkeypatch, object_test):
    ...
    assert object_test.questionning() == {
        "question": "Bonjour GrandPyBot ! Je recherche l'adresse d'Openclassrooms.",
        "wiki_call": "",
        "wiki_answ": {},
        "maps_call": [],
...
def test_cutfromlast():
    ...
    list_test = ["Je", "suis", "a", "la", "recherche", "des", "bureaux", "d",
    "Openclassrooms"]
    assert System.cut_from_last(list_test, STOP_WORDS) == \
    ["bureaux", "d", "Openclassrooms"]
...
def test_google_caller(monkeypatch, object_test):
    ...
    assert object_test.data["maps_answ"] == {
        "formatted_address": "Je connais bien cet endroit mon petit chat. "\
...
def test_wiki_no_caller(monkeypatch, object_test):
    """test wiki call if status == nothing"""
